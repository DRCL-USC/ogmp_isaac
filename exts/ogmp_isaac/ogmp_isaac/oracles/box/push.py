...
@dataclass
class PushReference(BaseReference):
...
class PushBoxOracle(BaseOracle):
    ...
    def _reach_reference(self, feedback):
        ...
        box_pos = feedback["box_pos"][env_ids, :2]

        yaw = self._compute_heading(robot_pos, box_pos)
    ...
    def _detach_reference(self, feedback):
        ...
        self._set_robot_reference(env_ids, robot_pos, robot_pos, torch.zeros((env_ids.numel(),), device=self.device))

        self.reference.box_pos[env_ids, :, :2] = detach_pos.unsqueeze(1).repeat(1, self.prediction_horizon, 1)
        self.reference.box_pos[env_ids, :, 2] = self.box_com

    def check_transitions(self, feedback):
        robot_pos = feedback["robot_pos"][:, :2]
        box_pos = feedback["box_pos"][:, :2]
        detach_pos = feedback["target_pos"]

        reach_dist = torch.linalg.norm(box_pos - robot_pos, dim=-1)
        detach_dist = torch.linalg.norm(box_pos - detach_pos, dim=-1)
